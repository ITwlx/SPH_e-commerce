      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              >
                ...
                <dd
                  ...
                  :key="index"
                  @click="getActive(spuSaleAttrValue,spuSaleAttr.spuSaleAttrValueList)"
                >
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="goodNum" @change="getGoodNum" />
                <a href="javascript:" class="plus" @click="goodNum++">+</a>
                <a href="javascript:" class="mins" @click="goodNum>1?goodNum--:goodNum=1">-</a>
              </div>
              <div class="add">
                <!-- 以前的路由跳转：从A路由跳转b路由，这里加入购物车，进行路由跳转之前，发送请求把
                购买的产品信息通过请求形式通知服务器，服务器进行响应的存储 -->
                <a @click="updateCartInfo">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    getActive(nowValue,attr){
      attr.forEach(item => {
        item.isChecked = 0
      });
      nowValue.isChecked = 1
    },
    // 从输入框获取购买商品数量
    getGoodNum(e){
      let newNum = e.target.value * 1
      if(isNaN(newNum) || newNum <1) this.goodNum = 1
      else {
        this.goodNum = parseInt(newNum)
      }
    },
    async updateCartInfo(){
      // 1.发送请求--将餐品添加到数据库（通知服务器）
       // this.$store.dispatch('getGoodUpdate',{skuId:this.$route.params.skuid,skuNum:this.goodNum})
      // 2.服务器存储成功--进行路由跳转传递参数 
      /*  派发action，向服务器发送数据，判断成功还是失败 方法加上async 返回一定是一个Promise
          要么成功，要么失败
      */
      try {
        await this.$store.dispatch('getGoodUpdate',{skuId:this.$route.params.skuid,skuNum:this.goodNum})
        // 成功并且进行路由跳转
        sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))
        this.$router.push({path:'/addcartlist',query:{skuId:this.goodNum}})
      } catch (error) {
        // 3.失败，给用户提示
        alert(error.message)
      }
